        ...
        <div>
                            ...
                            <div key={id} className="player-card">
                                <div className={`flip-card ${show ? "flipped" : ""}`}>
                                    <div className="flip-card-inner">
                                        <VoteIcon mostCommonVote={mostCommonVote} voteValue={voteValue}
                                                  show={room.showVotes}/>
            ...
            {isCurrentUserVoter() && (
                <div className="vote-buttons">
                    {[1, 2, 3, 5, 8, 13].map((v) => (
                        <button className="vote-button" key={v} onClick={() => vote(v)}
                                disabled={room.showVotes || !room.currentTicket}>
                            {v}
                        </button>
                    ))}
                    <button className="vote-action-button" key="clear" onClick={() => clearVote()}
                            disabled={room.showVotes || !room.currentTicket}>
                        Clear
                    </button>
                </div>
            ...
            {isCurrentUserDealer() && (
                <div className="vote-buttons">
                    <button className="vote-action-button" key="reset" onClick={() => resetVotes()}
                            disabled={(!room.isVoting || !room.currentTicket) && !room.showVotes}>
                        Reset Votes
                    </button>
                </div>
            )}
